...
std::mutex mut_img, mut_img1, mut_img2, mut_box, mut_yolo; // 여러개의 쓰레드에서 동시에 한 데이터에 접근하는 것을 막기 위한 뮤텍스
...
public:
  ...
  : Node("projection_box")
  {
    ...
    publish_point_ = this->create_publisher<sensor_msgs::msg::PointCloud2>(
      "/coord_xyz", rclcpp::SensorDataQoS());
...
public:
  ...
  static void * publish_thread(void * this_sub); // 메인 쓰레드
...
//=======================================클러스터 박스 가져와서 투영====================================
      ...
      for (int i = 0; i < boxes_2d_cluster.size(); i++)
      {
        ...
        if (max_iou > 0.2)
        {
          ...
          coord_cloud->push_back(coord_); // 포인트클라우드에 저장

        }
      }
      this_sub->publish_cone_->publish(coord); // float32multiarray
