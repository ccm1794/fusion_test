...
{
public:
  vector<double> extrinsic_param;
  vector<double> intrinsic_param;
  vector<double> dist_param;
  vector<int> img_size;
  ...
  : Node("fusion")
  {
    ...
    extrinsic_param = this->get_parameter("CameraExtrinsicMat").as_double_array();
    this->declare_parameter("CameraMat", vector<double>());
    intrinsic_param = this->get_parameter("CameraMat").as_double_array();
    this->declare_parameter("DistCoeff", vector<double>());
    dist_param = this->get_parameter("DistCoeff").as_double_array();
    this->declare_parameter("ImageSize", vector<int>());
    img_size = this->get_parameter("ImageSize").as_integer_array();
    ...
    //publish_thread();
    //pthread_create(&tids1_, NULL,  publish_thread, this);
...
void * ImageLiDARFusion::publish_thread(void * args)
{
  ...
  while(rclcpp::ok())
  {
    if(is_rec_image && is_rec_LiDAR)
    {
      ...
      pcl::toROSMsg(*pc_xyzinten, colored_msg);
      colored_msg.header.frame_id = "velodyne";
      colored_msg.header.stamp = rclcpp::Clock(RCL_ROS_TIME).now();
      LiDAR_pub->publish(colored_msg);
      loop_rate.sleep();
    }  
  }
  return 0;
